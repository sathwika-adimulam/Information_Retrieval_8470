...
class segment1(nn.Module):
    ...
    def forward(self, batch, item_emb):
        items = torch.tensor(batch['history_items'], dtype=torch.int64).to(self.device)  
        ...
        his_item_emb = (item_emb[items.transpose(0, 1)]*(~mask).transpose(0, 1).unsqueeze(2)) 
        x = self.transformer_encoder(his_item_emb, src_key_padding_mask=mask)  
        x = self.linear(x) 
        x = x * (~mask).transpose(0, 1).unsqueeze(2)
        user_emb_masked = x.sum(0)  
